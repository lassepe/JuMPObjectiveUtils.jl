...
"Represents an optimization objective that can be used by JuMP via `add_objective`."
abstract type AbstractJuMPObjective end
...
"""
struct NonlinearJuMPObjective{F} <: AbstractJuMPObjective
    cost::F
end
...
"""
struct QuadraticJuMPObjective{F} <: AbstractJuMPObjective
    cost::F
end
